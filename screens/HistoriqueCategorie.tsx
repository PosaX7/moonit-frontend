...
import { fetchTransactionsByCategorie, Transaction } from "../services/api";

type HistoriqueCategorieRouteProp = RouteProp<
...
export default function HistoriqueCategorie() {
  const route = useRoute<HistoriqueCategorieRouteProp>();
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await fetchTransactionsByCategorie(categorie);
        setTransactions(data);
      } catch (error) {
        console.error("Erreur chargement historique catégorie:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <View style={{ padding: 10 }}>
      <Text style={styles.title}>Historique de la catégorie : {categorie}</Text>
